...
/**
 * 이 Job은 {@code SCHEDULER_INTERVAL_SECONDS} 간격으로 외부 API(solved.ac API) 호출 요청이 담긴 대기열 큐를 확인하고, 요청을
 * 순차적으로 처리한다.
 * <p>
 ...
 * 메서드에서 메시지 타입(과제 할당 또는 과제 해결 여부 갱신 요청)에 따라 적절한 메서드를 호출하여 알고리즘 과제 관련 요청을 수행한다. </p>
 *
